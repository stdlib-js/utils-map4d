...
function map4d( arr, fcn, thisArg ) {
	...
	if ( !isArrayLikeObject( arr ) ) {
		throw new TypeError( format( '0kf2a', arr ) );
	}
	if ( !isFunction( fcn ) ) {
		throw new TypeError( format( '0kf2S', fcn ) );
	}
	S0 = arr.length;
	out = [];
	for ( i0 = 0; i0 < S0; i0++ ) {
		...
		if ( !isArrayLikeObject( a1 ) ) { // note: cannot support the more general "collections" here (which includes typed arrays having more than 2^32-1 elements), as the output array is limited to 2^32-1 elements; thus, we opt for the lowest common denominator: generic arrays
			throw new TypeError( format( '0kfCB', i0, a1 ) );
		}
		S1 = a1.length;
		t1 = [];
		for ( i1 = 0; i1 < S1; i1++ ) {
			...
			if ( !isArrayLikeObject( a2 ) ) {
				throw new TypeError( format( '0kfCC', i0, i1, a2 ) );
			}
			S2 = a2.length;
			t2 = [];
			for ( i2 = 0; i2 < S2; i2++ ) {
				...
				if ( !isArrayLikeObject( a3 ) ) {
					throw new TypeError( format( '0kfCD', i0, i1, i2, a3 ) );
				}
				S3 = a3.length;
				t3 = [];
				for ( i3 = 0; i3 < S3; i3++ ) {
